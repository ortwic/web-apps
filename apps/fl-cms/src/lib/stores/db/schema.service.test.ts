...
describe('schema store CRUD firebase tests', () => {
    ...
    it('should prevent timeouts by warming up firestore for very first test', async () => {
        const invalid = await firstValueFrom(scheme.getCollectionInternal('invalid'));
    ...
    it('should create collections with subcollections', async () => {
        ...
        let baz = await firstValueFrom(scheme.getCollectionInternal('foo', 'bar', 'baz'));
        ...
        // act #3
        foo = await firstValueFrom(scheme.getCollection('foo'));
        bar = await firstValueFrom(scheme.getCollection('foo/1/bar'));
        baz = await firstValueFrom(scheme.getCollection('foo/2/bar/3/baz'));
